...
if [ "$DAY_OF_WEEK" = "1" ] || [ "$BACKUP_STATUS" = "FAILED" ]; then
    # Monday = weekly detailed report, or any day if backup failed
    {
        ...
        if [ "$UPLOAD_SUCCESS" = true ] && [ "$VERIFY_SUCCESS" = true ]; then
            ...
            echo "2. Decrypt: sudo openssl enc -aes-256-cbc -d -pbkdf2 -in backup.tar.bz2.enc -out backup.tar.bz2 -pass file:/root/.backup-encryption-key"
            echo "3. Extract: sudo pbzip2 -dv -p16 -m1000 backup.tar.bz2"
            echo "4. Inflate: sudo tar -xvf backup.tar"
            echo "5. Prepare: sudo mariadb-backup --prepare --target-dir=backup-$DATE_STAMP"
            echo "6. Stop Monit: sudo systemctl stop monit"
            echo "7. Stop MariaDB: sudo systemctl stop mariadb"
            echo "8. Delete existing database files: sudo rm -rf /var/lib/mysql/*"
            echo "9. Restore: sudo mariadb-backup --copy-back --target-dir=backup-$DATE_STAMP"
            echo "10. Fix permissions: sudo chown -R mysql:mysql /var/lib/mysql"
            echo "11. Start MariaDB: sudo systemctl start mariadb"
            echo "12. Start Monit: sudo systemctl start monit"
        else
