...
if [ "$DAY_OF_WEEK" = "1" ] || [ "$BACKUP_STATUS" = "FAILED" ]; then
    # Monday = weekly detailed report, or any day if backup failed
    {
        ...
        if [ "$UPLOAD_SUCCESS" = true ] && [ "$VERIFY_SUCCESS" = true ]; then
            ...
            echo "5. Verify: mariadb-backup --verify --target-dir=backup-$DATE_STAMP"
            echo "6. Prepare: mariadb-backup --prepare --target-dir=backup-$DATE_STAMP"
            echo "7. Stop Monit: sudo systemctl stop monit"
